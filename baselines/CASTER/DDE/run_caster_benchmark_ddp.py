...
def parse_args():
    ...
    parser.add_argument('--predict_dim', type=int, default=500)
...
def test_dde_nn(loader, model, logger, device):
    ...
    for i, (v_d_poss, labels, v_d_neg_1s, v_d_neg_2s) in enumerate(loader):
        ...
        recons, codes, scores, z_fs, z_ds = model(v_ds)
        ...
        all_labels.extend(labels.repeat(3).cpu().flatten())
        all_preds.extend(torch.sigmoid(scores).squeeze().detach().cpu().flatten())
    
    return all_preds, all_labels, all_ys


def main(rank, world_size, args, output_dir, logger):
    ...
    #     'mag_factor': args.magnify_factor,
    # }
    ...
    train_loader = data.DataLoader(train_dataset, batch_size=batch_size, num_workers=num_workers, drop_last=False, sampler=train_sampler, pin_memory=False)

    if 'drugs' in split_method:
        val_between_dataset = supData(df_ddi_val_between, smiles_store, split='val_between')
        val_between_sampler = SequentialDistributedSampler(train_dataset, num_replicas=world_size, rank=rank, batch_size=batch_size)
        val_between_loader = data.DataLoader(val_between_dataset, batch_size=batch_size, num_workers=num_workers, drop_last=False, sampler=val_between_sampler, pin_memory=False)
        ...
        val_within_sampler = SequentialDistributedSampler(train_dataset, num_replicas=world_size, rank=rank, batch_size=batch_size)
        val_within_loader = data.DataLoader(val_within_dataset, batch_size=batch_size, num_workers=num_workers, drop_last=False, sampler=val_within_sampler, pin_memory=False)
        ...
        test_between_sampler = SequentialDistributedSampler(train_dataset, num_replicas=world_size, rank=rank, batch_size=batch_size)
        test_between_loader = data.DataLoader(test_between_dataset, batch_size=batch_size, num_workers=num_workers, drop_last=False, sampler=test_between_sampler, pin_memory=False)
        ...
        test_within_sampler = SequentialDistributedSampler(train_dataset, num_replicas=world_size, rank=rank, batch_size=batch_size)
        test_within_loader = data.DataLoader(test_within_dataset, batch_size=batch_size, num_workers=num_workers, drop_last=False, sampler=test_within_sampler, pin_memory=False)
        ...
        val_sampler = SequentialDistributedSampler(train_dataset, num_replicas=world_size, rank=rank, batch_size=batch_size)
        val_loader = data.DataLoader(val_dataset, batch_size=batch_size, num_workers=num_workers, drop_last=False, sampler=val_sampler, pin_memory=False)
        ...
        test_sampler = SequentialDistributedSampler(train_dataset, num_replicas=world_size, rank=rank, batch_size=batch_size)
        test_loader = data.DataLoader(test_dataset, batch_size=batch_size, num_workers=num_workers, drop_last=False, sampler=test_sampler, pin_memory=False)
        # test_loader = data.DataLoader(test_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)

    # print(len(train_dataset))
    # print(len(val_between_dataset))
    # print(len(val_within_dataset))
    ...
    for epoch in range(train_epoch):
        ...
        for i, (v_d_poss, labels, v_d_neg_1s, v_d_neg_2s) in enumerate(train_loader):
            # tic = time.perf_counter()
            logger.info(f"Train batch {i} / {len(train_loader)}")
            optim.zero_grad()

            # v_ds = torch.cat([v_d_poss, v_d_neg_1s, v_d_neg_2s], dim=0).float().to(device)
            v_ds = torch.cat([v_d_poss, v_d_neg_1s, v_d_neg_2s], dim=0).float()
            recons, codes, scores, z_fs, z_ds = model(v_ds)
    ...
    if 'drugs' in split_method:
        ...
        preds, labels, ys = test_dde_nn(test_within_loader, model, logger, rank)
        all_preds = [None for _ in range(world_size)]
        all_labels = [None for _ in range(world_size)]
        all_ys = [None for _ in range(world_size)]
        dist.all_gather(all_preds, preds)
        ...
        preds, labels, ys = test_dde_nn(test_loader, model, logger, rank)
        all_preds = [None for _ in range(world_size)]
        all_labels = [None for _ in range(world_size)]
        all_ys = [None for _ in range(world_size)]
        dist.all_gather(all_preds, preds)
...
if __name__ == '__main__':
    ...
    mp.spawn(
        main,
        args=(world_size, args, output_dir, logger),
        nprocs=world_size
    )
