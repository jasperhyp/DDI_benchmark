...
class dde_NN_Large_Predictor(nn.Sequential):
	...
	def __init__(self, **config):
		...
		self.predictor = nn.Sequential(
			...
			nn.BatchNorm1d(self.predict_dim),
   			# layer 6
			nn.Linear(self.predict_dim, self.predict_out_dim),
	...
	def dictionary_encoder(self, Z_D, Z_f, v_D):
		...
		# DTD_inv = torch.inverse(DTD + self.lambda3 * torch.eye(self.input_dim).cuda()) #.cuda()  # (D^T D + \lambda2 I )^{-1} D^T D, eta x eta
		DTD_inv = torch.pinverse(DTD)
	...
	def forward(self, v_D, labels):
		...
		score = self.predictor(self.mag_factor * code)[torch.arange(code.shape[0]), labels]
